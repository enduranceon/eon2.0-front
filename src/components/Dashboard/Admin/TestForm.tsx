...
export default function TestForm({ open, onClose, onSubmit, test, loading, error }: TestFormProps) {
  ...
  const { control, handleSubmit, reset, formState: { errors } } = useForm<TestFormData>({
    ...
    defaultValues: { name: '', description: '', type: TestType.PERFORMANCE, isActive: true },
  });
  ...
  useEffect(() => {
    if (open) {
      if (isEditMode && test) {
        ...
        reset({ name: '', description: '', type: TestType.PERFORMANCE, isActive: true });
      }
    }
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <DialogContent>
          ...
          <Grid container spacing={3} sx={{ mt: 1 }}>
            <Grid item xs={12}>
              <Controller name="name" control={control} render={({ field }) => ( <TextField {...field} label="Nome do Teste" fullWidth required error={!!errors.name} helperText={errors.name?.message} /> )}/>
            </Grid>
            <Grid item xs={12}>
